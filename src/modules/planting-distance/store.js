...
export const actions = {
  ...
  async fetchItems ({ commit, state }) {
    ...
    try {
      let response = await api.plantingDistance.getItems(state.query)
  ...
  createItem ({ commit, dispatch }, form) {
    return api.plantingDistance.createItem(form)
      .then(response => {
        dispatch('fetchItems')
  ...
  updateItem ({ commit, dispatch }, form) {
    return api.plantingDistance.updateItem(form)
      .then(response => {
        dispatch('fetchItems')
  ...
  deleteItem ({ dispatch }, id) {
    return api.plantingDistance.deleteItem(id)
      .then(response => {
        dispatch('fetchItems')
  ...
  fetchItem ({ commit, getters }, id) {
    return new Promise((resolve, reject) => {
      ...
      if (item) {
        ...
        api.plantingDistance.getItem({ id })
          .then(response => {
