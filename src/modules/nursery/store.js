...
export const actions = {
  ...
  async fetchItems ({ commit, state }) {
    ...
    try {
      let { items, meta } = await api.nursery.getFilteredItems({
        ...state.query
      })
  ...
  createItem ({ commit, dispatch }, form) {
    return api.nursery.createItem(form)
      .then(response => {
        dispatch('fetchItems')
  ...
  updateItem ({ commit, dispatch }, form) {
    return new Promise(async (resolve, reject) => {
      await api.nursery.updateItem(form)
        .catch(error => reject(error))

      await api.nursery.getFilteredItem({ id: form.id })
        .then(({ item }) => {
  ...
  deleteItem ({ dispatch }, id) {
    return api.nursery.deleteItem(id)
      .then(response => {
        dispatch('fetchItems')
  ...
  fetchItem ({ commit }, payload) {
    ...
    return new Promise((resolve, reject) => {
      if (item) {
        ...
        api.nursery.getFilteredItem({ id })
          .then(({ item }) => {
            commit('SET_ITEM', item)
  ...
  async uploadGalleryPhotos ({ dispatch }, payload) {
    try {
      ...
      data.append('nursery_id', data.get('id'))
      data.delete('id')
    ...
    try {
      await dispatch('fetchGallery', payload.data.get('nursery_id'))
