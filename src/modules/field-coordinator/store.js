...
export const actions = {
  ...
  async fetchItems ({ commit, state }) {
    ...
    try {
      let response = await api.fieldCoordinator.getItems(state.query)
  ...
  createItem ({ commit, dispatch }, form) {
    return api.fieldCoordinator.createItem(form)
      .then(response => {
        dispatch('fetchItems')
  ...
  updateItem ({ commit, dispatch }, form) {
    return api.fieldCoordinator.updateItem(form)
      .then(response => {
        dispatch('fetchItems')
  ...
  deleteItem ({ dispatch }, id) {
    return api.fieldCoordinator.deleteItem(id)
      .then(response => {
        dispatch('fetchItems')
  ...
  fetchItem ({ commit, getters }, id) {
    return new Promise((resolve, reject) => {
      ...
      if (item) {
        ...
        api.fieldCoordinator.getItem({ id })
          .then(response => {
